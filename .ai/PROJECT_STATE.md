...
> Stare branche robocze dewelopera `task/studio-007-port-hardening`, `task/studio-007-cmd-parentheses-fix` oraz `task/studio-007-fix2-goto-port-check` zostały pomyślnie scalone (metodą squash merge) i **nie są już gałęziami roboczymi (są nieaktywne)**. Wszelkie kolejne prace i ewentualne poprawki są realizowane na czystych, nowych gałęziach.
