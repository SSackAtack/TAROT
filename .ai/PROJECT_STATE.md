...
* **TASK-STUDIO-007-FIX (Czysty hotfix parsera nawiasów CMD w launcherze Studio):** Wyeliminowano błąd krytyczny deweloperski uniemożliwiający uruchomienie launchera w systemie Windows (objawiający się natychmiastowym zamykaniem okna konsoli zaraz po starcie). Przyczyną były nawiasy okrągłe w poleceniach `echo` wewnątrz bloku `if (...)` (np. `(Zalecane)`), które interpreter CMD błędnie brał za zamknięcie bloku instrukcji warunkowej. Zastąpienie ich przez nawiasy kwadratowe `[...]` przywróciło pełną stabilność parsowania. Zmiana została pomyślnie wprowadzona do mastera. **[APPROVED BY CHATGPT SUPERVISOR (PR #7 hotfix)]**
...
> Stare branche robocze dewelopera `task/studio-007-port-hardening` oraz `task/studio-007-cmd-parentheses-fix` zostały pomyślnie scalone (metodą squash merge) i **nie są już gałęziami roboczymi (są nieaktywne)**. Wszelkie kolejne prace i ewentualne poprawki są realizowane na czystych, nowych gałęziach.
