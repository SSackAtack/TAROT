...
def parse_control_message(raw_message):
    ...
    if message_type == "autotune_save":
        ...
        if not name:
            raise ControlMessageError("autotune_save requires non-empty name")
        import re
        if not re.match(r"^[a-zA-Z0-9_-]+$", name):
            raise ControlMessageError(f"Invalid profile name: '{name}'. Only letters, digits, underscore, and dash are allowed.")
        return ControlMessage(type=message_type, name=name)
